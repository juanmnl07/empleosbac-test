...
/*****************************************/
			...
			$.ajax({
			    ...
			    success: function (data) {
			    	if(data.estado == 1){
						...
						var id_puesto = pathArray[3];
						$.ajax({
						    method: "GET",
							url:"/usuario/aplicarPuesto/"+id_puesto,
							cache: false,
							contentType: false,
							processData: false,
						    error:function (jqXHR, textStatus, errorThrown) {
						    ...
						    success: function (data) { 
						    	...
						    	if(data.success == false){
						    		...
						    		if(data.code == 1){
						    			...
						    			$('#mensaje-despues-inicio-sesion').html('<p>&nbsp</p><p class="error message">Ya has aplicado para este puesto<br><br></p>');
									...
									$('#mensaje-despues-inicio-sesion').html('<h3>Has aplicado con <strong>éxito</strong> al puesto</h3>\
	...
	if($(".form").length > 0){
		...
		$('#perfil #boton-enviar').click(function(e){
			e.preventDefault();
			var paso = $(this).data("id");
			//console.log(validarPasoRegistro(paso));
			if(!validarPasoRegistro(paso)){
				$('#perfil').submit();
			}
			//$('.form .paso .siguiente').trigger('click');
		});
	...
	function validarPasoRegistro(paso){
		...
		switch(paso){
			case 'paso1':
				...
				}else if(validateEmail($(".contentform input#correo").val()) == false) {
					error_campo = true;
					mostrarMensajeError('Por favor, ingrese una dirección de correo electrónico válido');
					errorCampo(".contentform input#correo");
				}/* else if(($(".contentform input#clave_actual").length == 0) || ($(".contentform input#clave_actual").val() != '')){
				...
				if(error_campo == false){
					$('#perfil #boton-enviar').data('id', 'paso2');
				}*/
				else { /////////////////////////////////// COMENTADO POR ANDREY 30-11-2015 ///////////////////////////
					var email = $(".contentform input#correo").val();
					var typeform = $(".contentform input#correo").attr('class');
					var currentEmail = $(".contentform input#correo").attr('data');
					var a = $.ajax({
						async:false,
						url:"/rrhh/api/users/validate_email/retrieve.json?u_email=" + email,
					})
					.done(function(data) {
						//validar estado
						//si el tipo de formulario es 'perfil se validara a excepcion del correo actual del usuario'
						if(data.encontrado == true){
							if((typeform == 'perfil') && (currentEmail != email)){
								error_campo = true;
								...
								error_campo = true;
								mostrarMensajeError('Estimado usuario: el correo que utilizaste ya ha sido registrado. Puedes elegir otro correo o <a href="recordar" class="enlace-recordar">recuperar tu contraseña.</a>');
								errorCampo(".contentform input#correo");
							}
						}else{
				...
				request.open('GET', '/rrhh/api/users/validate_email/retrieve.json?u_email=' + email, false);  // `false` makes the request synchronous
				request.send(null);
				...
				if (request.status === 200) {
				  console.log(request.responseText);
					...
					/*$.holdReady( true );
					var a = $.get('/rrhh/api/users/validate_email/retrieve.json?u_email=' + email, function( data ) {
					  console.log(data);
					})
					.done(function(data) {
						console.log(data)
				...
				/*if (validarExistenciaEmail() == true){
					console.log(validarExistenciaEmail($(".contentform input#correo").val()));
					error_campo = true;
					mostrarMensajeError('Estimado usuario: el correo que utilizaste ya ha sido registrado. Puedes elegir otro correo o <a href="recordar" class="enlace-recordar">recuperar tu contraseña.</a>');
					errorCampo(".contentform input#correo");
				}*/
