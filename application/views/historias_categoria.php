<div class="w-section">
	<div class="w-container">
		<h1>Historias de liderazgo  - Categoria: <?php echo $nombre_categoria; ?></h1>
	...
	<div class="w-container">
		...
		<div class="articulos-listado-normal">
			...
			foreach ($listado_normal['results'] as $row) {
				...
				$colum .= '<div class="articulo-cat">'.$nombre_categoria.'</div>
			...
			if ($paginas_totales>0){
				if($pagina_actual>0){
					$html_paginas .= '<li><a href="/historias/categoria/'.$url_categoria.'/'.$id_categoria.'">&laquo;</a></li>';	
				}
				...
				for($i=$valor_inicial;$i<($paginas_por_mostrar+$valor_inicial);$i++){
					...
					if($i <= $paginas_totales){
						if($i==0){
							$html_paginas .= '<li '.$clase_current_page.'><a href="/historias/categoria/'.$url_categoria.'/'.$id_categoria.'">'.($i+1).'</a></li>';
						}else{
							$html_paginas .= '<li '.$clase_current_page.'><a href="/historias/categoria/'.$url_categoria.'/'.$id_categoria.'?pagina='.($i+1).'">'.($i+1).'</a></li>';
						}											
					}
				}

				if($pagina_actual<$paginas_totales){
					$html_paginas .= '<li><a href="/historias/categoria/'.$url_categoria.'/'.$id_categoria.'?pagina='.($paginas_totales+1).'">&raquo;</a></li>';
				}
				?>
