	...
	<div class="w-container">
		...
		<div class="articulos-listado-normal">
				...
				$colum .= '<div class="w-col w-col-[N]">
								<div class="articulo-box">
									<div class="articulo-imagen">
										<img src="'.$field_imagenes_superiores[0].'" />
									</div>
									<div class="articulo-cat">'.$field_categoria_blog[0].'</div>
			...
			if ($paginas_totales>0){
				if($pagina_actual>0){
					$html_paginas .= '<li><a href="/historias">&laquo;</a></li>';	
				}
				...
				for($i=$valor_inicial;$i<($paginas_por_mostrar+$valor_inicial);$i++){
					...
					if($i <= $paginas_totales){
						if($i==0){
							$html_paginas .= '<li '.$clase_current_page.'><a href="/historias">'.($i+1).'</a></li>';
						}else{
							$html_paginas .= '<li '.$clase_current_page.'><a href="/historias/page/'.($i+1).'">'.($i+1).'</a></li>';
						}											
					}
				}

				if($pagina_actual<$paginas_totales){
					$html_paginas .= '<li><a href="/historias/page/'.($paginas_totales+1).'">&raquo;</a></li>';
				}
				?>
